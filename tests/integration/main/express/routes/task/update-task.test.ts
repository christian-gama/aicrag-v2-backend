...
export default (): void =>
  describe('patch /task/:id', () => {
    ...
    it('should return 401 if user is not logged in', async () => {
      ...
      const result = await request(app).patch('/api/v1/task/any_id')
    ...
    it('should return 200 if all validations succeeds', async () => {
      ...
      expect(result.body.data.task.type).toBe('QA')
    }, 12000)
    ...
    it('should return 200 if there is no changes', async () => {
      ...
      expect(result.status).toBe(200)
      expect(result.body.data.message).toBe('No changes were made')
    }, 12000)
  })
