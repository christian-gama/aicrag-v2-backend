...
import { ValidateEmail } from '@/application/validators/query'
...
describe('validateEmail', () => {
  it('should return InvalidQueryError if email is not a string', async () => {
    const { request, sut } = makeSut()
    request.body.email = 123

    const result = await sut.validate(request.body)

    expect(result).toStrictEqual(new InvalidQueryError('email'))
  })

  it('should return a new InvalidQueryError if email is invalid', () => {
