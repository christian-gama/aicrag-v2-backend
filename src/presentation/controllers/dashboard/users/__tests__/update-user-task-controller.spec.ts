...
import { InvalidParamError, MustLoginError, TaskNotFoundError, UserNotFoundError } from '@/application/errors'
...
const makeSut = (): SutTypes => {
  ...
  const sut = new UpdateUserTaskController(
    httpHelper,
    taskRepositoryStub,
    userRepositoryStub,
...
describe('updateUserTaskController', () => {
  afterAll(() => {
    MockDate.reset()
  })
  ...
  it('should return badRequest if there is no task', async () => {
    ...
    jest.spyOn(taskRepositoryStub, 'superFindById').mockReturnValueOnce(Promise.resolve(null))
  ...
  it('should return task with new commentary if changes only commentary', async () => {
    ...
    jest.spyOn(taskRepositoryStub, 'superUpdateById').mockReturnValueOnce(Promise.resolve(updatedTask))
  ...
  it('should call superUpdateById with correct values if changes commentary', async () => {
    const { fakeTask, request, sut, taskRepositoryStub } = makeSut()
    const updateTaskSpy = jest.spyOn(taskRepositoryStub, 'superUpdateById')
  ...
  it('should call superUpdateById with correct values if changes date', async () => {
    const { fakeTask, request, sut, taskRepositoryStub } = makeSut()
    const updateTaskSpy = jest.spyOn(taskRepositoryStub, 'superUpdateById')
    const date = new Date()
  ...
  it('should call superUpdateById with correct values if changes duration', async () => {
    const { fakeTask, request, sut, taskRepositoryStub } = makeSut()
    const updateTaskSpy = jest.spyOn(taskRepositoryStub, 'superUpdateById')
  ...
  it('should call superUpdateById with correct values if changes status', async () => {
    const { fakeTask, request, sut, taskRepositoryStub } = makeSut()
    const updateTaskSpy = jest.spyOn(taskRepositoryStub, 'superUpdateById')
    request.body.status = 'in_progress'
  ...
  it('should return badRequest if is unable to update the task', async () => {
    const { httpHelper, request, sut, taskRepositoryStub } = makeSut()
    jest.spyOn(taskRepositoryStub, 'superUpdateById').mockReturnValueOnce(Promise.resolve(null))
  ...
  it('should call superUpdateById with correct values if changes taskId', async () => {
    const { fakeTask, request, sut, taskRepositoryStub } = makeSut()
    const updateTaskSpy = jest.spyOn(taskRepositoryStub, 'superUpdateById')
    request.body.taskId = 'any_value'
  ...
  it('should return UserNotFoundError if does not find a user that belongs to the task', async () => {
